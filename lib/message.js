...
const { sendInteractiveMessage, sendButtons, extractButtonData } = giftedBtns;
...
export function logic(check, inp, out) {
  if (inp.length !== out.length)
    throw new Error('The input and the output must have the same length.');
...
export async function smsg(client, m, store) {
  ...
  if (m.message) {
    ...
    // 🆕 Extract interactive button data
    m.buttonData = null;
    if (m.msg?.interactiveMessage?.nativeFlowMessage?.buttons) {
      m.buttonData = extractButtonData(m.msg);
    }
    
    // 🆕 Handle button responses (quick_reply, single_select, etc)
    if (m.msg?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson) {
      try {
        const json = JSON.parse(m.msg.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson);
        m.buttonResponse = {
          id: json.id || json?.reply?.id || '',
          selectedRowId: json.selectedRowId || null,
          displayText: json.display_text || json.selectedDisplayText || ''
        };
        m.body = m.buttonResponse.id || m.body;
      } catch (e) {}
    }
    
    // 🆕 Handle legacy button responses
    if (m.msg?.buttonsResponseMessage?.selectedButtonId) {
      m.buttonResponse = {
        id: m.msg.buttonsResponseMessage.selectedButtonId,
        displayText: m.msg.buttonsResponseMessage.selectedDisplayText || ''
      };
      m.body = m.buttonResponse.id || m.body;
    }
    
    // 🆕 Handle list responses
    if (m.msg?.listResponseMessage?.singleSelectReply?.selectedRowId) {
      m.buttonResponse = {
        id: m.msg.listResponseMessage.singleSelectReply.selectedRowId,
        displayText: m.msg.listResponseMessage.title || ''
      };
      m.body = m.buttonResponse.id || m.body;
    }
  ...
  client.sendInteractiveButtons = async (jid, content, options = {}) => {
    try {
      return await sendInteractiveMessage(client, jid, content, options);
    } catch (error) {
  ...
  // 🆕 Simple Buttons Helper
  client.sendButtons = async (jid, buttonsConfig) => {
    try {
      return await sendButtons(client, jid, buttonsConfig);
    } catch (error) {
  ...
  client.sendButton = async (
    ...
    if (/^https?:\/\//i.test(buffer)) {
      try {
        ...
        console.error('Error getting the MIME type:', error);
      }
    } else {
      try {
        ...
        console.error('Error getting the file type:', error);
      }
    }
